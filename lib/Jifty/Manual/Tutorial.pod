...
C<share/web/templates/>.  This default application is
...
As you might imagine by the fact that this tutorial application is
named "MyWebLog", we're building a simple weblog application. In future
tutorials, we'll add authentication, comments, RSS and Atom feeds.


=head3 Posts

Weblogs tend to center around posts, so it's no surprise that the first model
we'll be creating is the C<post>.
...
Open  C<lib/MyWeblog/Model/Post.pm> in your favorite text editor.
...
Now it's time to tell the model class about posts. We're going to start
by giving our post a C<body> and a C<title> and a C<category>. (In a future 
tutorial, we'll become fully folksonomy-compliant and upgrade that C<category>
to a C<tags> table.
...
Ok. It's time to initialize MyWeblog's database. By default, Jifty
sets up your application with the SQLite database engine.  If you'd
rather use PostgreSQL or MySQL, you'll need to add some content
to etc/jifty.yml. (Have a look at C<Jifty::Config> for a bit more
information).
...
Ok. You have a working, if simplistic, application.  Start up a
webserver and have a look around.  Be sure to check out the AJAX-enabled
administrative UI, online documentation browser and the Pony.
...
So, the administrative web does give you everything you need to work 
with your application's data, but it's not much of a weblog.
...
t's really easy to get a "basic" listing of entries and a little bit
more complex to get a pretty AJAXified paged list. We'll show you how
to do both and you can decide which one works best for you.


=head4 The quick and dirty way

Open a new file called "index.html" in your text editor. (Your webserver
will treat /index.html as the "default" page for your site).  Make it
look like this:
...
The "complex way" involves using one of Jifty's advanced features:
I<Page regions>. These regions let your application reload page sections
indepentently, either using AJAX on modern, high end, browsers or regular GET requests
with downlevel browsers like C<lynx>, C<w3m> or the browser on your mobile phone.
...
Open a new file called C<web/templates/index.html> in your text editor.  Fill it with this:
...
If you're on the ball, you've probably already guessed that you need to
create a file called C<web/templates/fragments/page_of_posts> and stuff it with this
...
And now, fire up your Jifty webserver again. Go create a post by browsing
B</post> on your webserver. 



=head2 Navigation

Of course, having to remember the URL to get to the posting page is
a bit annoying.  To get a "Post" button in the menu, you'll need to
override the default menus:

Jifty's "default" menus live in C<_elements/nav> in the default
application template (along with the Pony)

For now, you'll need to override C<_elements/nav>. (We're working on
ways to make this better.

Inside your applications C<web/templates> directory, create a directory
called C<_elements>.

Inside C<_elements/nav>, open up a new file called C<nav> in your text
editor and insert this content:
...
For more information about the menuing system, see the documentation
in L<Jifty::Menu>.

=head2

And that's just about everything you need to get started building Jifty
applications.  We're working hard to make Jifty even easier to use and
to obsolute the "hard bits" of this tutorial as quickly as we can.

Please join us on the C<jifty-devel> mailinglist to talk about how you're 
...
jifty-devel@lists.jifty.org is where we discuss how we're building Jifty, what
we're having trouble with and so on.
...
=head2 Access Control and Security
