...
{ # PARSE 
	...
	sub word {
		...
		if($word eq 'true'){
			$at += 3;
			next_chr;
			return $unmap ? 1 : bless {value => 'true'}, 'Jifty::JSON::NotString'
		}
		elsif($word eq 'null'){
			$at += 3;
			next_chr;
			return $unmap ? undef : bless {value => undef}, 'Jifty::JSON::NotString';
		}
		elsif($word eq 'fals'){
			$at += 3;
			if(substr($text,$at,1) eq 'e'){
				$at++;
				next_chr;
				return $unmap ? 0 : bless {value => 'false'}, 'Jifty::JSON::NotString'
			}
		}
