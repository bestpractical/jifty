...
pod2usage( -exitval => 0, -verbose => 2 , -input => $docs) if $OPTIONS->{man};
...
eval {
    Jifty->new(
        config_file => "$ProjectRoot/etc/config.yml",
...
if ( $OPTIONS->{tables} ) {
    ...
    my $appv =
        version->new( Jifty->framework_config('Database')->{'Version'} );
    ...
    for my $model ( __PACKAGE__->models ) {
        ...
        do { $log->info("Skipping $model"); next }
          if not UNIVERSAL::isa( $model, 'Jifty::Record' )
             or (UNIVERSAL::can( $model, 'since') and $appv < $model->since);
    ...
    eval {
        ...
        $upgrader->require();
        $UPGRADES{$_} = [$upgrader->upgrade_to($_)] for
          grep {$appv >= version->new($_) and $dbv < version->new($_)} $upgrader->versions();
    };
    ...
    for my $model ( __PACKAGE__->models ) {
            ...
            and $dbv < $model->since )
        {
            ...
            # Create it
            unshift @{$UPGRADES{$model->since}}, $t->sql_create_table( Jifty->handle->dbh );
        }
        else {
            ...
            for my $column ( $model->columns ) {
                    ...
                    and $dbv < $column->until )
                {
                    push @{$UPGRADES{$column->until}},
                      "ALTER TABLE " . $model->table . " DROP COLUMN " . $column->name;
                }
                    ...
                    and $dbv < $column->since )
                {
                    unshift @{$UPGRADES{$column->since}},
                      ...
                      . $model->table
                      . " ADD COLUMN "
                      . $t->column($column->name)->line( Jifty->handle->dbh );
                }
            }
        }
    }
    ...
    if ( $OPTIONS->{'print'} ) {
        for (map {@{$UPGRADES{$_}}} sort {version->new($a) <=> version->new($b)} keys %UPGRADES) {
            if (ref $_) {
...
sub create_db {
    ...
    my $database = Jifty->framework_config('Database')->{'Database'};
