	...
	if ($kid->name eq "anonlist" || $kid->name eq "anonhash") {
	    ...
	    return $pre . join(", ", @exprs) . $post;
		 ...
		 $kid->sibling->name eq "anoncode") {
            return $self->_anoncode($self->deparse_sub($self->padval($kid->sibling->targ)));
...
sub elem {
	...
	my $arrow = is_subscriptable($array) ? "" : $self->method_invocation;
...
sub method {
    ...
    my $args = join(", ", @exprs);	
    $kid = $obj . $self->method_invocation . $meth;
    if (length $args) {
...
sub pp_entersub {
	...
	$prefix = "";
	my $arrow = is_subscriptable($kid->first) ? "" : $self->method_invocation;
